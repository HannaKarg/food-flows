...
The dataset is available on zenodo.org. The [Jupyter Notebook] (/food-flows/blob/master/food_flow_notebook.ipynb) suggests options for reusing the data. The related Python script (food_flow_script.py, Version 3.9) requires the installation of following packages
	`pyproj`
	`matplotlib`
	`pandas`	
	`shapely`	
